		...
		if (Actor->IsALight()) {
			...
			if (Light->LightEffect == LE_Sunlight)
			{
				...
				for (INT LeafIndex = 0; LeafIndex < Model->Leaves.Num(); LeafIndex++)
				{
					...
					{
						LeafLights[LeafIndex] = new(GMem) FActorLink(Light, LeafLights[LeafIndex]);
						Count++;
					}
				}
...
void FEditorVisibility::TestVisibility()
{
	...
	for( INT Pass=0; Pass<2; Pass++ )
	{
		for( int i=0; i<Level->Actors.Num(); i++ )
		{
			...
			if (Actor && Actor->IsALight())
			{
				ALight* Light = Actor->GetLight();
				if (Light->LightType != LT_None && (Light->bStatic || Light->bNoDelete) && (!(RebuildOptions->Options & REBUILD_OnlyVisible) || !Actor->IsHiddenEd()) && Pass == 1)
				{
					GWarn->StatusUpdatef( NumLights[1], NumLights[0], TEXT("Illumination occluding") );
					INT NumVisibleLeaves = ActorVisibility(Light);
					debugf( NAME_Log, TEXT("Lightsource %s: %i leaves"), Light->GetName(), NumVisibleLeaves );
				}
				NumLights[Pass]++;
			}
		}
